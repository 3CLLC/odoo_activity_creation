...
class ResPartner(models.Model):
    ...
    def message_post(self, *args, **kwargs):
        ...
        message = super(ResPartner, self).message_post(*args, **kwargs)
    ...
    def _maybe_create_email_activity(self, message, subtype_xmlid):
        ...
        # Skip internal users - we don't want to track emails on user records
        if self.user_ids:
            _logger.info(f"DEBUG: Skipping internal user contact: {self.name}")
            return
    ...
    def _get_external_recipients(self, message):
        ...
        # For contact records, if no specific recipients found, use the contact itself
        # (since we're sending an email from the contact's chatter to that contact)
        if not recipient_emails:
            # Use the contact's email or name as the recipient
            email = self.email or self.name
            if email:
                recipient_emails.append(email)
                _logger.info(f"DEBUG: Added contact as recipient: {email}")
